...
{ 
	...
	vec3 lookfrom = vec3(20*cos(angle), 1, 20*sin(angle));
    vec3 lookat = vec3(0, 0, 0);
    vec3 vup = vec3(0, 1, 0);
    float dist_to_focus = 10.0;
    float aperture = 0.1;
    camera = CameraConstructor(lookfrom, lookat, vup, 30, aspect_ratio, aperture, dist_to_focus);
